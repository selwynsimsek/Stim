...
namespace clasp_stim {
  ...
  GateTarget target_x_target(const GateTarget &t, bool invert){
  ...
  GateTarget target_y_target(const GateTarget &t, bool invert){
    if (!t.is_qubit_target()) {
      throw std::invalid_argument("result of stim.target_x(" + t.str() + ") is not defined");
    }
  ...
  GateTarget target_z_target(const GateTarget &t, bool invert){
    if (!t.is_qubit_target()) {
      throw std::invalid_argument("result of stim.target_x(" + t.str() + ") is not defined");
    }
  ...
  std::vector<GateTarget> target_combined_paulis_string(const FlexPauliString &ps, bool invert){
    std::vector<GateTarget> result;
    if (ps.imag) {
      std::stringstream ss;
  ...
  GateTarget target_pauli_string(uint32_t qubit_index, const std::string &p, bool invert){
  ...
  GateTarget target_pauli_uint8_t(uint32_t qubit_index, const uint8_t &p, bool invert){
    if ((qubit_index & TARGET_VALUE_MASK) != qubit_index) {
      std::stringstream ss;
      ss << "qubit_index=" << qubit_index << " is too large. Maximum qubit index is " << TARGET_VALUE_MASK << ".";
      throw std::invalid_argument(ss.str());
